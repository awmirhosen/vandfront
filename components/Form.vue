      ...
      <div class="w-full ">
        ...
        <FormKit type="form"
          ...
          <div class="mt-10"
            ...
            <FormKit class="w-full discipline"
            ...
            <div class="flex items-end">
              ...
              <div class="bg-black text-white whitespace-nowrap py-2 px-4 cursor-pointer"
                   @click="sendEmailToken">
                {{ shareStore.process.action === "send_email_token" ? 'Sending...' : 'Send Code' }}
              </div>
            ...
            <label>
              <input type="checkbox"
                     v-model="formStore.formData.policy"
                     class="accent-zinc-900 w-4 h-4 mr-2">
...
const previewImage = (e) => {
  ...
  if (input.files) {
    if (Math.floor(input.files[0].size / 1024) > 4000) {
      shareStore.setToast("File size is too big!", true);
    } else {
...
const submitForm = async () => {
  ...
  if (!formStore.formData.policy) {
    shareStore.setToast("Please confirm the Term.", true);
    return;
  }
